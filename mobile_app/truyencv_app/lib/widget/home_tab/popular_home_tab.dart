...
class PopularHomeTab extends StatefulWidget {
  const PopularHomeTab({
    Key? key,
    required this.size,
  }) : super(key: key);
  ...
  @override
  State<PopularHomeTab> createState() => _PopularHomeTabState();
}

final _base = ApiUrls().baseUrl;
Future<List<Book>> fetchPopular() async {
  final response =
      await http.get(Uri.parse('$_base/tcv/public/api/v1/popular'));
...
class _PopularHomeTabState extends State<PopularHomeTab> {
  late Future<List<Book>> futurePopular;
    ...
    return SizedBox(
      ...
      child: FutureBuilder<List<Book>>(
        ...
        builder: (context, snapshot) {
          ...
          return Container(
            width: widget.size.width - 20,
            height: (widget.size.width - 20) * 1.2,
